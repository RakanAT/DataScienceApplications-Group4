...
**Assignment 2 (Clustering):** Four feature representations (BOW, TF-IDF, LDA, Sentence Embeddings) are clustered with K-Means, EM (GMM), and Hierarchical clustering, then evaluated against the true genre labels.
...
| 6 | `claire-Clustering.ipynb` | Clusters the same partitions (no labels used during fitting) across 4 feature sets × 3 algorithms, evaluates with Silhouette, Kappa, and Coherence |
...
**Champion model:** Hierarchical clustering on sentence embeddings (`all-MiniLM-L6-v2`), Kappa = 0.627 — unchanged even after adding BOW as a 4th feature set.
